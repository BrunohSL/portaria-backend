...
for secret in portaria_db_password portaria_db_root_password portaria_jwt_secret portaria_encryption_key portaria_metrics_token portaria_twilio_account_sid portaria_twilio_auth_token portaria_openai_api_key portaria_elevenlabs_api_key; do
  if [ -f "/run/secrets/$secret" ]; then
    case "$secret" in
      ...
      portaria_twilio_account_sid)  export TWILIO_ACCOUNT_SID=$(cat /run/secrets/$secret) ;;
